...
def _make_problem(*, n_residuals, n_params):
    ...
    ys = jnp.sin(design @ theta_true)
    x = jnp.zeros(n_params)

    def residual(theta, args, p):
        design, ys = args
        return jnp.sin(design @ theta) - ys

    solver = UnderdeterminedLevenbergMarquardt(residual, init_damping=1e-2)
    lm_state = solver.init(x, (design, ys))

    @jax.jit
    def step(x, lm_state):
        return solver.update(x, lm_state, (design, ys))

    return x, lm_state, step


def _benchmark_update(benchmark, *, n_residuals, n_params):
    x, lm_state, step = _make_problem(n_residuals=n_residuals, n_params=n_params)
